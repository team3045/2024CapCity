  ...
  private void configureBindings() {
    drivetrain.setDefaultCommand(
        drivetrain.getDriveCommand(
            () -> -joystick.getLeftY() * CommandSwerveDrivetrain.MaxSpeed, // TODO: REVERSE X AND Y FOR REAL BOT BUT SIM
                                                                           // JOYSTICKLS ARE BEING WEIRD
    ...
    // TODO: Should set the arm to start aiming / tracking target, should have
    // drivetrain start aiming towards target
    // Should have shooters rev, Essentially sets everything up for the later
    // trigger
    joystick.triangle().toggleOnTrue(
        shooter.setRevving()
            .alongWith(
                drivetrain.aimAtSpeakerMoving(
                    () -> -joystick.getLeftY() * CommandSwerveDrivetrain.MaxSpeed, // TODO: SEE COMMENT ABOVE ON OTHER
                                                                                   // DRIVE COMMAND
                    () -> -joystick.getLeftX() * CommandSwerveDrivetrain.MaxSpeed))
    ...
    shooter.isRevving
        ...
        .whileTrue(shooter.setShooting().andThen(shooter.feedNote())); // TODO: add a cancel so we go back to normal
                                                                       // rotation maybe

    // Once the note is gone we're done shooting so we go idle and coast the
    // shooters
